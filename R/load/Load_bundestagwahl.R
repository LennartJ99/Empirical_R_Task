...
Election<-read.csv(here("input","btw2021kre-is.csv"), sep=";", header=TRUE, encoding="UTF-8",skip=9)
Election<-Election%>%
...
###To solve this Problem we transform the comma to a point, comma and convert to numeric.
extract_clean_convert <- function(x) {
  cleaned <- gsub(",", ".", x)  # Remove commas
  X <- as.numeric(cleaned)
  # Convert to numeric
  return(X)
}
